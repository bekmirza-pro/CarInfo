...
exports.UserCreate = async(req, res, next)=>{
    if(!Auth){
        ...
        const user = new Auth({name: req.body.name, email: req.body.email, password: req.body.password})
        await user.save()
...
exports.UserLogin = async function(req, res, next){
    ...
    const result = await Auth.findOne({email: email}).select("password").select("type")
    if(!result){
        console.log("err")
    }else{
